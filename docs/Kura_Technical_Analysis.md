...
### ĐỀ TÀI: ỨNG DỤNG QUẢN LÝ VÀ ĐỒNG HÀNH SỐ KURA
...
| **Hình 4.4** | Giao diện Dashboard Con hiển thị thời gian khả dụng và Quota | Chương 4 |
...
| :--- | :--- | :--- |
| **Bảng 1.1** | Bảng câu hỏi phỏng vấn và khảo sát thực tế | Thu thập nhu cầu của Phụ huynh & Con |
...
| **Bảng 2.5** | Bảng đặc tả Use Case UC-05: Gửi Yêu cầu Xin thêm giờ | Con gửi request gia hạn thời gian |
| **Bảng 2.6** | Bảng đặc tả Use Case UC-06: Phê duyệt Yêu cầu & Auto-Approval | Phụ huynh duyệt hoặc hệ thống tự động duyệt |
...
Bước vào kỷ nguyên chuyển đổi số, điện thoại thông minh và máy tính bảng đã trở thành công cụ học tập và giải trí không thể thiếu đối với các con. Tuy nhiên, mặt trái của sự bùng nổ công nghệ là tình trạng nghiện mạng xã hội (TikTok, Facebook, Instagram), game online, cùng với nguy cơ tiếp xúc với các nội dung độc hại (bạo lực học đường, cờ bạc trực tuyến, lừa đảo và các trào lưu tự gây thương tích). 

Các giải pháp giám sát con cái hiện nay trên thị trường thường gặp phải ba vấn đề lớn:
1. **Tính độc đoán và thụ động:** Chỉ tập trung vào việc chặn đứng một chiều (Blacklist blocking) mà thiếu đi kênh tương tác, thảo luận bình đẳng giữa phụ huynh và con cái, dẫn tới việc con tìm cách gỡ bỏ hoặc gian lận (bật VPN, chỉnh giờ hệ thống).
...
Ứng dụng **Kura** được nghiên cứu và thiết kế nhằm giải quyết triệt để các hạn chế trên. Kura là giải pháp quản trị và đồng hành số toàn diện:
- Cung cấp cơ chế khóa thông minh (**Smart Lock**) kết hợp kênh đối thoại hai chiều (**Time Request**) cho phép con chủ động xin thêm thời gian học tập.
...
Báo cáo kỹ thuật này thể hiện trọn vẹn quy trình nghiên cứu, phân tích thiết kế hướng đối tượng và hiện thực hóa phần mềm Kura – sản phẩm công nghệ sáng tạo của con THPT nhằm mang lại giải pháp đồng hành số thực tiễn và an toàn cho gia đình.
...
1. **Kỹ thuật phỏng vấn trực tiếp (In-depth Interview):** Tiến hành phỏng vấn sâu 15 phụ huynh có con học bậc THCS và THPT cùng 15 con về thói quen sử dụng điện thoại, cảm giác khi bị phụ huynh cấm đoán và nguyện vọng được cấp quyền tự chủ.
2. **Khảo sát diện rộng bằng biểu mẫu (Online Survey):** Thu thập 120 phiếu trả lời khảo sát từ các nhóm cộng đồng phụ huynh và các con tại Hà Nội.
...
| :--- | :--- | :--- | :--- |
| **1** | Phụ huynh muốn giám sát thiết bị của con theo hình thức nào? | Phụ huynh | Cần một ứng dụng di động duy nhất, phân tách 2 giao diện (Parent Dashboard và Child Mode) dựa trên vai trò tài khoản được xác thực qua Firebase Auth. |
| **2** | Khi con đang sử dụng app giải trí quá giờ quy định, phần mềm phải làm gì? | Phụ huynh | Hệ thống phải lập tức đẩy ứng dụng cấm xuống nền bằng lệnh `GLOBAL_ACTION_HOME` trong thời gian < 0.5s và hiển thị màn hình khóa cảnh báo đè lên. |
| **3** | Con cảm thấy khó khăn nhất điều gì khi bị phụ huynh khóa máy đột ngột? | Con | Bị ngắt kết nối khi đang trao đổi bài tập nhóm. Cần có nút "Xin thêm giờ" (15p, 30p) kèm lý do để phụ huynh nhận thông báo và bấm duyệt từ xa. |
| **4** | Làm cách nào để phụ huynh biết con đang tìm kiếm các nội dung tiêu cực (tự tử, bạo lực)? | Phụ huynh | Ứng dụng phải ngầm duyệt cây giao diện (`AccessibilityNodeInfo`) trên các ứng dụng mạng xã hội và trình duyệt để phát hiện từ khóa nguy hiểm và gửi cảnh báo đỏ. |
...
| **6** | Gia đình có nhiều con thì quản lý ra sao? | Phụ huynh | Hệ thống áp dụng cấu trúc `Family Group` với mã kết nối `Link Code` 6 ký tự. Một tài khoản phụ huynh có thể quản lý nhiều thiết bị con độc lập. |
...
| **AppTimeLimits** | Cấu hình giới hạn thời gian (phút/ngày) cho từng ứng dụng của từng con. | Firestore `timeLimits` & Native Prefs |
...
1. **Hiệu năng chặn ứng dụng tức thì:** Khi con mở một ứng dụng đã hết hạn mức hoặc trong giờ giới nghiêm, thời gian từ lúc ứng dụng mở đến lúc bị đẩy ra Home phải **nhỏ hơn 500ms** để triệt tiêu cảm giác giật/lag hoặc khả năng con lách luật bấm nhanh.
...
```mermaid
flowchart LR
 subgraph Kura_System ["HỆ THỐNG KURA"]
...
Phân hệ xử lý vòng đời yêu cầu xin thêm giờ của con và sự phản hồi của phụ huynh:
...
- **Tác nhân:** Phụ huynh (Parent), Con (Child) 
...
- **Luồng sự kiện chính:**
 ...
 b. Con nhập mã 6 số do phụ huynh cung cấp và nhấn "Kết nối".
 c. Hệ thống tìm kiếm trong Firestore xem mã `linkCode` có hợp lệ không.
 d. Hệ thống cập nhật trường `familyId` cho con, đồng thời thêm UID của con vào mảng `childUids` của gia đình.
 e. Ứng dụng trên máy con chuyển sang màn hình hướng dẫn cấp quyền Trợ năng (Accessibility).
- **Luồng ngoại lệ:**
...
- **Luồng sự kiện chính:**
 1. Phụ huynh chọn tên con cần thiết lập hạn mức.
 ...
 7. Firestore Realtime Stream phát tín hiệu cập nhật tới máy con.
 8. Máy con nhận payload và lập tức lưu đè vào bộ nhớ cục bộ `SharedPreferences` ở tầng Native.
- **Luồng ngoại lệ:**
...
- **Luồng sự kiện chính:**
 ...
 6. Quy tắc được đồng bộ tự động xuống máy con và nạp vào danh sách kiểm soát của `AppMonitorService`.
...
- **Mục đích:** Đảm bảo khi con vi phạm quy tắc giới hạn hoặc lịch trình, ứng dụng cấm sẽ bị đẩy văng lập tức và màn hình cảnh báo hiển thị. 
- **Điều kiện tiên quyết:** Dịch vụ Trợ năng `AppMonitorService` đã được kích hoạt trên thiết bị của con. 
- **Sự kiện kích hoạt:** Con bấm vào biểu tượng mở một ứng dụng (ví dụ: Facebook). 
- **Luồng sự kiện chính:**
...
- **Luồng ngoại lệ:**
 - *Con cố tình nhấn mở lại app cấm:* Vòng lặp kiểm tra nhận diện ngay trong 50ms và tiếp tục đẩy văng ra Home, ngăn chặn hoàn toàn việc truy cập trái phép.
...
- **Tác nhân:** Con (Child) 
- **Mục đích:** Cho phép con xin phép phụ huynh gia hạn thêm thời gian sử dụng khi có việc cần thiết chính đáng. 
- **Điều kiện tiên quyết:** Ứng dụng của con đang bị màn hình khóa chặn lại. 
- **Luồng sự kiện chính:**
 1. Tại màn hình `LockScreen`, con nhấn nút "Xin thêm giờ".
 2. Hộp thoại `RequestTimeDialog` xuất hiện.
 3. Con chọn thời gian mong muốn (15 phút, 30 phút, hoặc 60 phút).
 4. Con chọn lý do trong danh sách mẫu ("Học nhóm", "Tra cứu bài tập") hoặc tự nhập văn bản ngắn.
 5. Nhấn "Gửi yêu cầu".
 6. Hệ thống thêm bản ghi vào `families/{familyId}/children/{childUid}/timeRequests` với trạng thái `status: 'pending'`.
 7. Màn hình thông báo cho con: "Đã gửi yêu cầu tới phụ huynh. Vui lòng chờ phản hồi!".
- **Luồng rẽ nhánh (Tự động duyệt):**
 - Hệ thống kiểm tra quy tắc `autoApprovalRules`. Nếu thời gian xin <= 15 phút và trong khung giờ cho phép, hệ thống tự động chuyển trạng thái thành `approved` mà không cần phụ huynh thao tác thủ công.
...
- **Luồng sự kiện chính:**
 1. Firebase Cloud Functions phát hiện Document mới tạo trong `timeRequests` và gửi Push Notification có âm thanh ưu tiên cao tới máy phụ huynh.
...
```mermaid
flowchart TD
 Start([Con mở một ứng dụng]) --> CatchEvent[HĐH Android phát sự kiện AccessibilityEvent]
...
```mermaid
flowchart TD
 StartReq([Ứng dụng bị khóa]) --> ClickReq[Con bấm 'Xin thêm giờ']
...
flowchart TD
 ...
 TriggerFunc --> PushUrgentFCM[Bắn FCM độ ưu tiên cao tới điện thoại Phụ huynh]
 PushUrgentFCM --> RingAlarm[Điện thoại Phụ huynh rung chuông báo động đỏ]
...
flowchart TD
 ...
 SaveFamily --> DisplayCode[Hiển thị mã 6 số to rõ trên Dashboard phụ huynh]
 
 CheckRole -- Con --> EnterCode[Con nhập mã 6 số từ phụ huynh]
...
sequenceDiagram
 ...
 LockScreen-->>Child: Thông báo: 'Phụ huynh đã duyệt thêm 30 phút'
```
...
sequenceDiagram
 autonumber
 actor Child as Con
 participant TargetApp as App trên máy Con (YouTube/Chrome)
...
erDiagram
 ...
 string childUid FK "ID của con chịu giới hạn"
 string appName "Tên hiển thị thân thiện"
 map limits "Giới hạn số phút theo các thứ trong tuần"
 boolean isBlocked "Cờ khóa tức thì do phụ huynh gạt tay"
 }

 SCHEDULE {
 string scheduleId PK "ID của lịch trình"
 string childUid FK "Áp dụng cho con nào"
 ...
 string childUid FK "Con gửi yêu cầu"
 ...
 string childUid FK "Con liên quan"
 ...
 boolean isReviewed "Đã được phụ huynh xem xét chưa"
...
- **Khả năng tự vận hành ngoại tuyến (Offline Resilience):** Khi điện thoại của con không có mạng, quy tắc giới hạn vẫn được thực thi đầy đủ nhờ bộ đệm `SharedPreferences`.
...
classDiagram
    class AppMonitorService {
        ...
        +performGlobalAction(Int action) Boolean
        +saveBlockedAppsToPrefs(Context context) void
    ...
    class AccessibilityChannel {
        ...
        +startMonitorService() Future~void~
        +stopMonitorService() Future~void~
        ...
        +updateMonitoredPackages(List~String~ packages) Future~void~
        +moveToHome() Future~void~
        +isAccessibilityPermissionGranted() Future~bool~
        +openAccessibilitySettings() Future~void~
        +getAndClearOfflineUsageLogs() Future~List~
    }
    ...
    class AppMonitorBloc {
        ...
        +onAppEventReceived(AppEventReceived event) void
        +onKeywordDetected(KeywordDetectedEvent event) void
        +onMidnightRollover(MidnightRolloverTriggered event) void
    }
...
classDiagram
    class SmartLockBloc {
        -SmartLockRepository repository
        +onLoadAppTimeLimits(LoadAppTimeLimits event) void
        +onSaveAppTimeLimit(SaveAppTimeLimit event) void
        +onLoadMonitoredApps(LoadMonitoredApps event) void
        +onToggleMonitoredApp(ToggleMonitoredApp event) void
        +onLoadSchedules(LoadSchedules event) void
        +onSaveSchedule(SaveSchedule event) void
        +onDeleteSchedule(DeleteSchedule event) void
        +onLoadSmartLockSettings(LoadSmartLockSettings event) void
        +onSaveSmartLockSettings(SaveSmartLockSettings event) void
        +onLoadLockHistory(LoadLockHistory event) void
    }

    class SmartLockRepository {
        -FirebaseFirestore _firestore
        +getFamily(String familyId) Future~FamilyModel~
        +getAppTimeLimits(String familyId, String childId) Future~List~AppTimeLimitModel~~
        +watchTimeLimits(String familyId, String childId) Stream~List~AppTimeLimitModel~~
        +saveAppTimeLimit(String familyId, String childId, AppTimeLimitModel limit) Future~void~
        +getSchedules(String familyId, String childId) Future~List~ScheduleModel~~
        +watchSchedules(String familyId, String childId) Stream~List~ScheduleModel~~
        +saveSchedule(String familyId, String childId, ScheduleModel schedule) Future~void~
        +getSmartLockSettings(String familyId) Future~SmartLockSettingsModel~
        +saveSmartLockSettings(String familyId, SmartLockSettingsModel settings) Future~void~
        +getMonitoredApps(String familyId, String childId) Future~List~MonitoredAppModel~~
    }
    ...
    class AppTimeLimitModel {
        ...
        +bool isBlocked
        +fromJson(Map~String, dynamic~ json) AppTimeLimitModel
    ...
    class ScheduleModel {
        ...
        +fromFirestore(DocumentSnapshot doc) ScheduleModel
        +toMap() Map~String, dynamic~
    }

    class SmartLockSettingsModel {
        +String familyId
        +bool isEnabled
        ...
        +int quietHoursStart
        +int quietHoursEnd
        +fromFirestore(DocumentSnapshot doc) SmartLockSettingsModel
        +toMap() Map~String, dynamic~
    }
    ...
    class MonitoredAppModel {
        ...
        +String iconUrl
        +bool isMonitored
        +fromJson(Map~String, dynamic~ json) MonitoredAppModel
        +toJson() Map~String, dynamic~
    }

    SmartLockBloc --> SmartLockRepository : Gọi nghiệp vụ CSDL
    SmartLockRepository --> AppTimeLimitModel : Quản lý
    SmartLockRepository --> ScheduleModel : Quản lý
    SmartLockRepository --> SmartLockSettingsModel : Quản lý
    SmartLockRepository --> MonitoredAppModel : Quản lý
```
...
classDiagram
    class TimeRequestBloc {
        ...
        +submitRequest(TimeRequest request) void
        +approveRequest(String familyId, String childUid, String requestId, String response) void
        +rejectRequest(String familyId, String childUid, String requestId, String response) void
        +watchPendingRequests(String familyId) void
    }
    ...
    class TimeRequestRepository {
        ...
        +watchAllRequests(String familyId) Stream~List~TimeRequest~~
        ...
        +countRecentRequests(String familyId, String childUid, String appPackageName, Duration window) Future~int~
    }
    ...
    class AutoApprovalRule {
        ...
        +fromFirestore(DocumentSnapshot doc) AutoApprovalRule
        +toMap() Map~String, dynamic~
    }

    TimeRequestBloc --> TimeRequestRepository : Sử dụng
    TimeRequestRepository <|.. TimeRequestRepositoryImpl : Triển khai
    TimeRequestRepositoryImpl --> TimeRequest : Xử lý thực thể
    TimeRequest --> TimeRequestStatus : Trạng thái
    TimeRequestBloc --> AutoApprovalRule : Đánh giá tự động duyệt
```
...
classDiagram
    class AlertRepository {
        <<Interface>>
        +createKeywordAlert(String familyId, String childUid, String keyword, String packageName, String textContext) Future~void~
        +createAppBlockedAlert(String familyId, String childUid, String packageName, String reason) Future~void~
        +createTimeRequestAlert(String familyId, String childUid, String packageName, int requestedMinutes) Future~void~
        +watchNewAlerts(String familyId, String childUid) Stream~List~AlertModel~~
        +watchAllAlerts(String familyId, String childUid) Stream~List~AlertModel~~
        +watchAllFamilyAlerts(String familyId) Stream~List~AlertModel~~
        +getAlert(String familyId, String childUid, String alertId) Future~AlertModel~
        +markAlertAsReviewed(String familyId, String childUid, String alertId) Future~void~
        +addNotesToAlert(String familyId, String childUid, String alertId, String notes) Future~void~
        +dismissAlert(String familyId, String childUid, String alertId) Future~void~
        +watchKeywords(String familyId) Stream~List~String~~
    }
    ...
    class InAppNotificationBloc {
        ...
        +onLoadInAppNotifications(LoadInAppNotifications event) void
        +onMarkAsRead(MarkInAppNotificationAsRead event) void
        +onMarkAllAsRead(MarkAllInAppNotificationsAsRead event) void
    }

    class KeywordManagementBloc {
        -FirebaseFirestore _firestore
        -AccessibilityChannel _channel
        +onLoadKeywords(LoadKeywords event) void
        +onAddKeyword(AddKeyword event) void
        +onRemoveKeyword(RemoveKeyword event) void
        +onResetToDefaults(ResetToDefaults event) void
    }

    AlertRepository <|.. AlertRepositoryImpl : Kế thừa
    InAppNotificationBloc --> AlertRepository : Lắng nghe
    InAppNotificationBloc --> AlertModel : Hiển thị
    KeywordManagementBloc ..> AlertRepository : Đồng bộ từ khóa
...
erDiagram
    ...
    FAMILIES ||--o{ AUTO_APPROVAL_RULES : "cấu hình tự động duyệt"
    ...
    CHILDREN_SUB ||--o{ MONITORED_APPS : "danh mục app giám sát"
    CHILDREN_SUB ||--o{ USAGE_LOGS : "sinh ra nhật ký sử dụng"
    CHILDREN_SUB ||--o{ DAILY_SUMMARIES : "kết xuất tổng kết ngày"

    FAMILIES {
        string id PK "families/{familyId}"
        string linkCode "Mã ghép nối 6 chữ số"
        string parentUid "UID của phụ huynh tạo gia đình"
        array childUids "Danh sách các UID của con"
    ...
    TIME_LIMITS {
        ...
        string appName "Tên ứng dụng"
        map limits "Giới hạn theo thứ"
    ...
    TIME_REQUESTS {
        ...
        int requestedMinutes "Số phút xin thêm"
        string reason "Lý do con nhập"
        string status "pending, approved, rejected"
        string parentResponse "Lời nhắn phản hồi của phụ huynh"
    ...
    MONITORED_APPS {
        string appPackageName PK "monitoredApps/{appPackageName}"
        string appName "Tên ứng dụng"
        string iconUrl "Đường dẫn icon"
        boolean isMonitored "Đang bật giám sát"
    }

    AUTO_APPROVAL_RULES {
        string id PK "settings/autoApprovalRules"
        string familyId "Khóa ngoại gia đình"
        int maxAutoApproveMinutes "Thời lượng tối đa tự duyệt"
        int dailyAutoApproveLimit "Số lần tự duyệt tối đa mỗi ngày"
        boolean isEnabled "Bật tắt tự động duyệt"
        map appSpecificRules "Quy tắc riêng từng app"
    }
    ...
    SMART_LOCK_SETTINGS {
        ...
        int quietHoursStart "Giờ bắt đầu ngủ đêm"
        int quietHoursEnd "Giờ kết thúc ngủ đêm"
    }

    USAGE_LOGS {
        string docId PK "usage_logs/{docId}"
        string childUid "UID của con"
        string familyId "ID gia đình"
        string appPackage "Package name ứng dụng"
        string appName "Tên hiển thị"
        int durationMinutes "Số phút sử dụng"
        string date "Ngày sử dụng YYYY-MM-DD"
    }

    DAILY_SUMMARIES {
        string summaryId PK "daily_summaries/{summaryId}"
        string childUid "UID của con"
        string familyId "ID gia đình"
        string date "Ngày tổng kết YYYY-MM-DD"
        int totalMinutes "Tổng số phút dùng trong ngày"
        map usageByApp "Map thời lượng theo app"
        array topApps "Danh sách top ứng dụng"
    }
```
...
| `parentUid` | String | Có | UID của phụ huynh | Chủ sở hữu gia đình có toàn quyền quản trị |
| `childUids` | Array~String~ | Có | Mảng danh sách UID | Danh sách các con thuộc nhóm gia đình |
| `createdAt` | Timestamp | Có | `serverTimestamp()` | Thời điểm tạo gia đình |

---

##### Bảng 3.3: Chi tiết Sub-collection `timeLimits`
- **Mô tả:** Cấu hình giới hạn thời gian cho từng ứng dụng cụ thể của từng con.
...
- **Mô tả:** Nhật ký các yêu cầu xin thêm giờ của con và phản hồi từ phụ huynh.
...
| `childUid` | String | Có | Khóa ngoại | UID của con gửi yêu cầu |
...
| `requestedMinutes` | Int | Có | 15, 30, hoặc 60 | Số phút con xin cấp thêm |
| `reason` | String | Không | Tối đa 200 ký tự | Lý do con cung cấp |
...
| `packageName` | String | Có | App phát sinh sự kiện | Nơi con thực hiện hành vi tra cứu |
| `textContext` | String | Không | Đoạn text trích xuất | Ngữ cảnh câu văn bản chứa từ khóa |
| `isReviewed` | Boolean | Có | Mặc định: `false` | Đánh dấu phụ huynh đã xem hay chưa |
| `isDismissed` | Boolean | Có | Mặc định: `false` | Đánh dấu đã xóa/ẩn cảnh báo |
| `notes` | String | Không | Ghi chú của phụ huynh | Nhận xét biện pháp giáo dục của phụ huynh |
...
| Tên trường | Kiểu dữ liệu | Bắt buộc | Ràng buộc / Giá trị mẫu | Diễn giải ý nghĩa nghiệp vụ |
| :--- | :--- | :--- | :--- | :--- |
| `childUid` | String | Có | Khóa ngoại | Con phát sinh phiên sử dụng |
...
- **Hình 4.1: Giao diện Đăng nhập & Xác thực Firebase Auth**
 - Giao diện thiết kế theo phong cách hiện đại, hỗ trợ chuyển đổi linh hoạt giữa đăng nhập Phụ huynh và Con. Tích hợp xác thực email chuẩn bảo mật.
...
- **Hình 4.2: Giao diện Ghép đôi thiết bị gia đình qua Link Code**
 - Màn hình phụ huynh tự động sinh mã số lớn, rõ ràng gồm 6 ký tự. Màn hình con cung cấp ô nhập PIN 6 số tự động focus và kiểm tra tính hợp lệ tức thời.
...
- **Hình 4.4: Giao diện Dashboard Con**
 - Thân thiện, hiển thị số phút còn lại cho từng app giải trí, đồng hồ đếm ngược và trạng thái kết nối với phụ huynh.
...
- **Hình 4.5: Giao diện Toàn màn hình khóa Native (LockScreen Overlay)**
 - Tự động bật lên đè kín màn hình ngay khi con mở ứng dụng cấm, chặn mọi thao tác chạm vào ứng dụng bên dưới.
...
- **Hình 4.6: Giao diện Cấu hình Giới hạn Ứng dụng & Lịch trình**
 - Cho phép phụ huynh kéo thanh trượt chọn số phút từ 15 phút đến 240 phút cho từng ngày trong tuần.
...
- **Hình 4.8: Giao diện Trung tâm Cảnh báo Từ khóa nhạy cảm**
 - Hiển thị danh sách các từ ngữ nguy hiểm mà con đã gõ trên mạng, kèm thời gian, biểu tượng mức độ rủi ro và ô nhập ghi chú của phụ huynh.
...
| **Chức năng Xin thêm giờ** | Con xin giờ -> Phụ huynh nhận thông báo -> Duyệt | Đạt 100% | Cloud Functions v2 gửi FCM trong 1.8 giây |
...
| Tiêu chí so sánh | Google Family Link | Qustodio Parental Control | Kura |
| :--- | :--- | :--- | :--- |
...
Qua quá trình nghiên cứu, phân tích thiết kế và triển khai thực nghiệm, đề tài **"Phần mềm Quản lý và Đồng hành số Kura"** đã đạt được các kết quả xuất sắc:
...
3. **Cơ chế Gamification (Thưởng điểm tích cực):** Cho phép con cái tích lũy thời gian học tập hữu ích để đổi lấy hạn mức chơi game giải trí, biến việc quản trị thiết bị thành một trò chơi đồng hành tích cực giữa phụ huynh và con.
